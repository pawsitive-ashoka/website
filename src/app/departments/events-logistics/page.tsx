"use client";

import { useState } from "react";

export default function EventsLogisticsPage() {
  const [expandedItem, setExpandedItem] = useState<number | null>(null);

  const events = [
    {
      title: "Event/Drive 1",
      image: "/events/event1.jpg", // Placeholder - add actual image
      description: "Description of this event or drive will be added here. Include details about what happened, when it took place, and its impact on the campus animals.",
    },
    {
      title: "Event/Drive 2",
      image: "/events/event2.jpg", // Placeholder - add actual image
      description: "Description of this event or drive will be added here. Include details about what happened, when it took place, and its impact on the campus animals.",
    },
    {
      title: "Event/Drive 3",
      image: "/events/event3.jpg", // Placeholder - add actual image
      description: "Description of this event or drive will be added here. Include details about what happened, when it took place, and its impact on the campus animals.",
    },
    // Add more events as needed
  ];

  const toggleExpand = (index: number) => {
    setExpandedItem(expandedItem === index ? null : index);
  };

  return (
    <main className="mx-auto max-w-6xl px-6 py-16">
      <h1 className="mb-4 text-4xl font-bold text-amber-900">
        Events and Logistics
      </h1>
      <p className="mb-12 text-lg text-amber-800">
        {/* Department description will be added here */}
        Short description of the Events and Logistics department will be added here.
      </p>

      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {events.map((event, index) => (
          <div
            key={index}
            className="rounded-2xl bg-white/60 shadow-sm backdrop-blur-sm overflow-hidden transition-all hover:shadow-lg"
          >
            {/* Event Image */}
            <div
              className="relative h-48 w-full bg-gradient-to-br from-amber-100 to-orange-100 flex items-center justify-center cursor-pointer"
              onClick={() => toggleExpand(index)}
            >
              <span className="text-6xl">📸</span>
              {/* Replace with actual image when available:
              <Image 
                src={event.image} 
                alt={event.title}
                fill
                className="object-cover"
              /> */}
            </div>

            {/* Event Title and Description */}
            <div className="p-6">
              <h2
                className="mb-3 text-xl font-bold text-amber-900 cursor-pointer hover:text-amber-700"
                onClick={() => toggleExpand(index)}
              >
                {event.title}
              </h2>

              {/* Expandable Description */}
              {expandedItem === index && (
                <div className="mt-4 pt-4 border-t border-amber-200">
                  <p className="text-sm text-amber-800/90">{event.description}</p>
                </div>
              )}

              <button
                onClick={() => toggleExpand(index)}
                className="mt-4 text-sm font-semibold text-amber-900 hover:text-amber-700"
              >
                {expandedItem === index ? "Show Less ▲" : "Learn More ▼"}
              </button>
            </div>
          </div>
        ))}
      </div>
    </main>
  );
}
